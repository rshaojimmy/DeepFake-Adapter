...
PRETAINED_WEIGHT_PATH = "/mnt/lustre/rshao/program/deepfake_detection/Ours/models/xception-b5690688.pth"
...
class TransferModel(nn.Module):
    ...
    def __init__(self, modelchoice, num_out_classes=2, dropout=0.0, weight_norm=False, return_fea=False, inc=3):
        ...
        if modelchoice == "xception":
            ...
            def return_pytorch04_xception(pretrained=True):
                ...
                if pretrained:
                    ...
                    print(f"load pre-trained weights from {PRETAINED_WEIGHT_PATH}")
            ...
            if not dropout:
                if weight_norm:
                    ...
                    self.model.last_linear = nn.utils.weight_norm(nn.Linear(num_ftrs, num_out_classes), name="weight")
                ...
                if weight_norm:
                    ...
                    self.model.last_linear = nn.Sequential(
                        ...
                        nn.utils.weight_norm(nn.Linear(num_ftrs, num_out_classes), name="weight"),
                    )

                self.model.last_linear = nn.Sequential(nn.Dropout(p=dropout), nn.Linear(num_ftrs, num_out_classes))
    ...
    def set_trainable_up_to(self, boolean=False, layername="Conv2d_4a_3x3"):
        ...
        if boolean:
            ...
            for name, child in self.model.named_children():
                ...
                ct.append(name)
            if not found:
                raise NotImplementedError("Layer not found, cant finetune!".format(layername))
        else:
...
def model_selection(modelname, num_out_classes, dropout=None):
    ...
    if modelname == "xception":
        ...
        return (
            TransferModel(modelchoice="resnet18", dropout=dropout, num_out_classes=num_out_classes),
            224,
            True,
            ["image"],
            None,
...
#     model = TransferModel('xception', num_out_classes=1, dropout=0.5)
#     print(model)
#     # model = model.cuda()

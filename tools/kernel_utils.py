...
def predict_on_video(
    face_extractor, video_path, batch_size, input_size, models, strategy=np.mean, apply_compression=False
):
    batch_size *= 4

    try:
        faces = face_extractor.process_video(video_path)
        print("{} resolution: ({}, {})".format(video_path.split("/")[-1], faces[0]["frame_w"], faces[0]["frame_h"]))
        if len(faces) > 0:
            ...
            for frame_data in faces:
                face_cnt += 1
                print("{} faces {}  : {}".format(video_path.split("/")[-1], face_cnt, len(frame_data["faces"])))
                print("{} scores    : {}".format(video_path.split("/")[-1], frame_data["scores"]))
            ...
            if n > 0:
                ...
                for i in range(len(x)):
                    x[i] = normalize_transform(x[i] / 255.0)
...
def predict_on_video_set(
    ...
    def process_file(i):
        ...
        print("prediction on {}: {}".format(filename, y_pred))
        return y_pred
